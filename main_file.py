...
from Add_Emp_details import Add_Emp_assert_details
...
if __name__ == "__main__":
    with st.sidebar:
        choose = option_menu("Details", ["Emp_details", "Asset_details", "Add_details"])
    # ---------------------EMP-Details-------------------------#
    if choose == "Emp_details":
        ...
        if Emp_det == "Name":
            ...
            name = st.selectbox("Enter Employee Name: ", name_list)
            if name == name_list[0]:
                pass
            else:
                ...
                try:
                    if st.session_state['button'] == True:
                        ...
                        if st.button('Send Mail'):
                            if re.search("^[a-z].*@thundersoft.com$", receiver_ID):
        ...
        # ---------------------Asset_details-------------------------#
    elif choose == "Asset_details":
        client = set_image()
        # --------------------TS---------------------------------#
        if client == client_list[0]:
            Asset_det = st.selectbox("Get Details Through:", ("Asset Id", "Device"))
